...
| [`spec-writing`](skills/spec-writing) | Turns a rough idea, a premade brief, or a Q&A session into a SPEC.md decomposable into unambiguous, dependency-ordered tasks — picking a flat/phased/staged-DAG weight class to match project size — then files each task as a GitHub issue via `github-issue-filing`. |
