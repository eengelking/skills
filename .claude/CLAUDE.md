...
| Review (pre-PR) | Before opening the PR, run `/code-review` on the branch and apply its findings with `--fix`, so the branch's own commit history — and the PR that follows — already reflects reviewed code rather than a later "address review comments" commit. Reserve post-PR `--comment`/`ultra` review for a second pass (e.g. a deeper multi-agent review, or a check the user explicitly wants visible as PR comments) rather than the default path. | `code-review` |
